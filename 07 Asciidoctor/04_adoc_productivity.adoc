...
Lade die Datei *link:asciidoc-productivity/asciidoc-productivity-1.0.2.vsix[asciidoc-productivity-1.0.2.vsix]* in einen Ordner auf deiner Festplatte.
...
=== 2. TSV-Tabellen einfügen (Insert as table)
Kopiere tabellenartige Daten (z. B. direkt aus Excel) in deine Zwischenablage. Wähle im Rechtsklick-Menü `Insert as table`. 
Die Extension erkennt automatisch die Anzahl der Spalten (anhand der Tabulatoren) und generiert einen fertigen AsciiDoc-Tabellen-Block.
